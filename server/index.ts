...
subRedis.on("error", (err) => {
  console.error("Redus subscription error", err);
});
...
io.on("connection", async (socket) => {
  ...
  socket.on("join-room", async (room: string) => {
    ...
    const subscribeRooms = await redis.smembers("subscribe-rooms");

    await socket.join(room);
    await redis.sadd(`rooms:${id}`, room);
    await redis.hincrby("room-connections", room, 1);

    if (!subscribeRooms.includes(room)) {
      subRedis.subscribe(room, async (err) => {
        if (err) {
          console.error("Failed to subscribe: ", err);
        } else {
          await redis.sadd("subscribe-rooms", room);
      ...
      if (remainingConnections <= 0) {
        ...
        subRedis.unsubscribe(room, async (err) => {
          if (err) {
            ...
            await redis.srem("subscribe-rooms", room);
            console.log("Unsubscribe from room: ", room);
          }
        });
      }
    });
